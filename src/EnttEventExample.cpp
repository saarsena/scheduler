...
void EnttEventExample::onEntityAttack(const GameEvents::EntityAttackEvent &event) {
    std::cout << "Attack event: Entity " << static_cast<int>(event.attacker) << " attacks "
              << static_cast<int>(event.target) << " for " << event.damage << " damage";
...
void EnttEventExample::onEntityDamaged(const GameEvents::EntityDamagedEvent &event) {
    std::cout << "Damage event: Entity " << static_cast<int>(event.entity) << " takes "
              << event.damage << " damage of type " << event.damageType;
...
void EnttEventExample::onEntityDied(const GameEvents::EntityDiedEvent &event) {
    std::cout << "Death event: Entity " << static_cast<int>(event.entity) << " died";
...
void EnttEventExample::onPlayerMove(const GameEvents::PlayerMoveEvent &event) {
    std::cout << "Player " << static_cast<int>(event.player) << " moved from (" << event.fromX
              << "," << event.fromY << ") to (" << event.toX << "," << event.toY << ")"
              << std::endl;
}
...
void EnttEventExample::runEnttDispatcherExample() {
    ...
    dispatcher.sink<GameEvents::EntityAttackEvent>().connect<&EnttEventExample::onEntityAttack>();
    dispatcher.sink<GameEvents::EntityDamagedEvent>().connect<&EnttEventExample::onEntityDamaged>();
    dispatcher.sink<GameEvents::EntityDiedEvent>().connect<&EnttEventExample::onEntityDied>();
    dispatcher.sink<GameEvents::PlayerMoveEvent>().connect<&EnttEventExample::onPlayerMove>();
    dispatcher.sink<GameEvents::MapChangeEvent>().connect<&EnttEventExample::onMapChange>();
    ...
    // Enemy takes damage
    dispatcher.enqueue<GameEvents::EntityDamagedEvent>(enemy, 15, player, "physical");

    // Player critical attack
    dispatcher.enqueue<GameEvents::EntityAttackEvent>(player, enemy, 30, true);

    // Enemy takes more damage
    dispatcher.enqueue<GameEvents::EntityDamagedEvent>(enemy, 30, player, "physical");
...
void EnttEventExample::runCombinedExample() {
    std::cout << "\n=== Combined Scheduler and Event Dispatcher Example ===" << std::endl;
    ...
    dispatcher.enqueue<GameEvents::EntityDamagedEvent>(enemy, 25, player, "physical");
